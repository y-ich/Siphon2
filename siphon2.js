...
(function() {
  var AutoComplete, BOUNDARY, CLIENT_ID, COFFEE_KEYWORDS, CORE_CLASSES, DATE_PROPERTIES, JS_KEYWORDS, KEYWORDS, KEYWORDS_COMPLETE, OPERATORS, OPERATORS_WITH_EQUAL, SCOPES, UTC_PROPERTIES, classes, close_delim, delimiter, downloadFile, e, functions, getList, globalProperties, globalPropertiesPlusKeywords, newCodeMirror, prepareTable, uploadFile, variables, _i, _j, _len, _len1, _ref, _ref1;
  ...
  */
  ...
  getList = function() {
    return googleDrive.getList(['.html', '.css', '.js', '.less', '.coffee'].map(function(e) {
  ...
  $('#download~ul').on('click', 'a', function(event) {
    ...
    return file = new googleDrive.File($(this).data('resource').download(function(text) {
  ...
  uploadFile = function() {
    ...
    if (file != null) {
      return file.update(null, $active.data('editor').getValue());
    } else {
      return googleDrive.File.insert($active.text(), 'text/plain', $active.data('editor').getValue());
    }
  };
